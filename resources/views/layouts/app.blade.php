    ...
    <body>
        <header class="p-5 border-b bg-white shadow">
            <div class="container mx-auto flex justify-between items-center">
                @auth
                    <a href="{{route('teams.index', auth()->user()->name)}}" class="text-3xl font-black hidden md:block">
                    ...
                    <nav class="flex gap-2 items-center flex-wrap md:flex-nowrap">
                        ...
                        @if ($user->role == 1) {{-- Usuario --}}
                            ...
                            <a href="{{route('journeys.index', auth()->user()->name)}}"
                            class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            ...
                            <a href="{{route('journeys.index', auth()->user()->name)}}"
                            class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold cursor-pointer">
                            <img width="24" height="24" src="https://img.icons8.com/wired/64/grid.png" alt="grid"/>
                                Tablas
                            </a>
